...
@graph.post("/triples/import")
async def import_graph_triples(file: UploadFile = File(...), current_user: User = Depends(get_admin_user)):
    """Import curated graph triples as draft records."""
    try:
        content = await file.read()
        service = GraphTripleService()
        result = await service.import_file(file.filename or "triples.jsonl", content, current_user.user_id)
        return {"success": True, "data": result}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Failed to import graph triples: {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Failed to import graph triples: {str(e)}")


@graph.get("/triples")
async def list_graph_triples(
    status: str | None = Query(None),
    source_kind: str | None = Query(None),
    limit: int = Query(100, ge=1, le=500),
    current_user: User = Depends(get_admin_user),
):
    """List graph triple drafts for review."""
    try:
        service = GraphTripleService()
        return {"success": True, "data": await service.list_triples(status, source_kind, limit)}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Failed to list graph triples: {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Failed to list graph triples: {str(e)}")


@graph.post("/triples/{triple_id}/review")
async def review_graph_triple(
    triple_id: str,
    action: str = Body(...),
    review_comment: str | None = Body(None),
    embed_model_name: str | None = Body(None),
    batch_size: int | None = Body(None),
    current_user: User = Depends(get_admin_user),
):
    """Approve or reject a staged graph triple. Approved triples are written to Neo4j."""
    try:
        service = GraphTripleService()
        reviewed = await service.review_triple(triple_id, action, current_user.user_id, review_comment)
        if reviewed["status"] == "approved":
            await graph_base.txt_add_vector_entity(
                [GraphTripleService.to_graph_payload(reviewed)],
                "neo4j",
        ...
        logger.error(f"Failed to review graph triple: {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Failed to review graph triple: {str(e)}")
...
async def add_neo4j_entities(
    ...
    """Compatibility endpoint: import MinIO triple files as drafts, not final graph data."""
    try:
        ...
        service = GraphTripleService()
        result = await service.import_file(object_name, content, current_user.user_id)
        return {
            "success": True,
            "message": "Triples imported as drafts; approve them before writing to the graph",
            "status": "draft",
            "data": result,
        }
        ...
        logger.error(f"Failed to import graph triple drafts: {e}, {traceback.format_exc()}")
        return {"success": False, "message": f"Failed to import graph triple drafts: {e}", "status": "failed"}
