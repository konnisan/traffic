...
async def create_database(
    ...
    try:
        ...
        requested_kb_type = kb_type
        kb_type = kb_type_aliases.get((kb_type or "").lower(), kb_type)

        additional_params = {**(additional_params or {})}
        additional_params.setdefault("rag_mode", requested_kb_type)
        additional_params.setdefault("graph_extraction", False)
...
async def get_knowledge_base_types(current_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        kb_types["text_rag"] = {
            "class_name": "MilvusKB",
            "description": "文本检索知识库，复用 Milvus 向量检索，默认不抽取知识图谱",
            "default_config": {"maps_to": "milvus", "graph_extraction": False},
        }
        kb_types["graph_rag"] = {
            "class_name": "ReviewedTriplesGraph",
            "description": "人工审核三元组图谱入口，不从任意文件自动生成正式关系",
            "default_config": {"maps_to": "milvus", "graph_extraction": False},
        }
        kb_types["visual_rag"] = {
            "class_name": "VisualRagPoC",
            "description": "视觉文档页级检索 PoC，适合扫描件、表格和版式强 PDF",
            "default_config": {"standalone_api": "/api/knowledge/visual-rag"},
        }
...
@knowledge.post("/visual-rag/index")
async def create_visual_rag_index(file: UploadFile = File(...), current_user: User = Depends(get_admin_user)):
    """Create a small local visual RAG page index for a PDF document."""
    try:
        content = await file.read()
        result = await VisualRagService().index_document(file.filename or "document.pdf", content)
        return {"success": True, "data": result}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Visual RAG indexing failed: {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Visual RAG indexing failed: {str(e)}")


@knowledge.post("/visual-rag/query")
async def query_visual_rag(
    index_id: str = Body(...),
    query: str = Body(...),
    top_k: int = Body(5),
    current_user: User = Depends(get_required_user),
):
    """Query a visual RAG page index and return page-level evidence."""
    try:
        result = await VisualRagService().query(index_id=index_id, query=query, top_k=top_k)
        return {"success": True, "data": result}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Visual RAG query failed: {e}, {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Visual RAG query failed: {str(e)}")
