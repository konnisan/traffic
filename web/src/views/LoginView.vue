  ...
  <div class="login-view" :class="{ 'has-alert': serverStatus === 'error' }">
    ...
    <nav class="login-navbar">
      <div class="navbar-content">
        <div class="brand-container" @click="goHome" style="cursor: pointer">
          <shield-check-icon class="brand-logo-icon" size="28" />
          <h1 class="brand-text">
            <span class="brand-org">收费稽核</span>
            <span class="brand-separator"></span>
            <span class="brand-main">交通稽核智能体</span>
          </h1>
        </div>
      </div>
    </nav>
      ...
      <div class="login-card">
        ...
        <div class="card-side is-image">
          <div class="audit-login-panel">
            <a-tag color="blue">辅助研判 · 人工复核</a-tag>
            <h2>交通稽核工作台</h2>
            <p>聚焦路径还原、异常证据、政策依据和报告草稿生成。业务事实由确定性规则计算，智能体只做解释和辅助。</p>
            <div class="audit-login-metrics">
              <div>
                <strong>路径还原</strong>
                <span>入口 / 门架 / 出口</span>
              </div>
        ...
        <div class="card-side is-form">
          <div class="form-wrapper">
            <header class="form-header">
              <p class="welcome-text">TRAFFIC AUDIT</p>
              <!-- 如果是在初始化，显示特定标题 -->
              <h2 v-if="isFirstRun" class="init-title">系统初始化，请创建超级管理员</h2>
              <h2 v-else class="init-title">登录交通稽核工作台</h2>
            </header>
    ...
    <footer class="page-footer">
      <div class="footer-links">
        <span>业务流水不进入 RAG 或图谱</span>
        <span class="divider">|</span>
        <span>正式结论必须人工确认</span>
      </div>
      <div class="copyright">
        &copy; {{ new Date().getFullYear() }} 交通稽核智能体. All Rights Reserved.
      </div>
    </footer>
  </div>
</template>
...
.card-side.is-image {
  flex: 1.4;
  background:
    radial-gradient(circle at top left, var(--main-100), transparent 34%),
    linear-gradient(135deg, var(--main-20), var(--main-5));
  overflow: hidden;
}
...
.audit-login-metrics strong {
  display: block;
  color: var(--main-700);
}

.audit-login-metrics span {
  display: block;
  margin-top: 4px;
  color: var(--gray-600);
  font-size: 13px;
