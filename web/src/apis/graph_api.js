import { apiGet, apiPost, apiRequest } from './base'
...
export const neo4jApi = {
  ...
  importTripleDrafts: async (file) => {
    const formData = new FormData()
    formData.append('file', file)
    return await apiRequest(
      '/api/graph/triples/import',
      {
        method: 'POST',
        body: formData
      },
  ...
  listTripleDrafts: async (params = {}) => {
    const queryParams = new URLSearchParams()
    if (params.status) queryParams.set('status', params.status)
    if (params.source_kind) queryParams.set('source_kind', params.source_kind)
    if (params.limit) queryParams.set('limit', String(params.limit))
    const query = queryParams.toString()
    return await apiGet(`/api/graph/triples${query ? `?${query}` : ''}`, {}, true)
  },

  reviewTripleDraft: async (tripleId, payload) => {
    return await apiPost(`/api/graph/triples/${tripleId}/review`, payload, {}, true)
  }
}
