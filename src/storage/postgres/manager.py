...
class PostgresManager(metaclass=SingletonMeta):
    ...
    async def ensure_knowledge_schema(self):
        ...
        stmts = [
            ...
            """
            CREATE TABLE IF NOT EXISTS graph_triple_drafts (
                ...
                source_kind VARCHAR(64) DEFAULT 'curated_triples',
            ...
            "CREATE INDEX IF NOT EXISTS idx_gtd_status ON graph_triple_drafts(status)",
            "CREATE INDEX IF NOT EXISTS idx_gtd_source_doc ON graph_triple_drafts(source_doc_id)",
            "CREATE INDEX IF NOT EXISTS idx_gtd_subject ON graph_triple_drafts(subject)",
        ]
    ...
    async def ensure_business_schema(self):
        ...
        stmts = [
            ...
            CREATE TABLE IF NOT EXISTS dify_workflow_runs (
                ...
                case_id VARCHAR(32) NOT NULL REFERENCES traffic_audit_cases(id),
            ...
            "CREATE INDEX IF NOT EXISTS idx_dify_workflow_case_created ON dify_workflow_runs(case_id, created_at DESC)",
            "CREATE INDEX IF NOT EXISTS idx_dify_workflow_department_created "
            "ON dify_workflow_runs(department_id, created_at DESC)",
            "CREATE INDEX IF NOT EXISTS idx_dify_workflow_status ON dify_workflow_runs(status)",
            "CREATE INDEX IF NOT EXISTS ix_conversations_is_pinned ON conversations(is_pinned)",
            "CREATE INDEX IF NOT EXISTS idx_traffic_audit_department_created "
            "ON traffic_audit_cases(department_id, created_at DESC)",
        ]
        async with self.async_engine.begin() as conn:
            for stmt in stmts:
                await conn.execute(text(stmt))
