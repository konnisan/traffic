...
class TrafficAuditCase(Base):
    """交通稽核案件及其一期样例数据。"""

    __tablename__ = "traffic_audit_cases"
    ...
    def to_dict(self, include_datasets: bool = False) -> dict[str, Any]:
        result = {
            "id": self.id,
            "department_id": self.department_id,
            "title": self.title,
            ...
            "status": self.status,
            "created_by": self.created_by,
            "dataset_summary": {key: len(value.get("records", [])) for key, value in (self.datasets or {}).items()},
        ...
        if include_datasets:
            result["datasets"] = self.datasets or {}
        return result
...
class DifyWorkflowRun(Base):
    ...
    id = Column(String(32), primary_key=True)
    case_id = Column(String(32), ForeignKey("traffic_audit_cases.id"), nullable=False, index=True)
    department_id = Column(Integer, ForeignKey("departments.id"), nullable=False, index=True)
    workflow_code = Column(String(64), nullable=False, index=True)
