...
class GraphTripleDraft(Base):
    """Human-reviewed graph triple staging model."""

    __tablename__ = "graph_triple_drafts"
    __table_args__ = (UniqueConstraint("triple_id", name="uq_graph_triple_drafts_triple_id"),)

    id = Column(Integer, primary_key=True, autoincrement=True)
    triple_id = Column(String(64), unique=True, nullable=False, index=True)
    subject = Column(String(512), nullable=False, index=True)
    predicate = Column(String(256), nullable=False, index=True)
    object = Column(String(512), nullable=False, index=True)
    subject_type = Column(String(128))
    object_type = Column(String(128))
    source_doc_id = Column(String(256), index=True)
    source_page = Column(Integer)
    source_quote = Column(Text)
    confidence = Column(Float, default=1.0)
    source_kind = Column(String(64), default="curated_triples", index=True)
